...
logger.add("logs/bot.log", format="{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {name}:{function}:{line} - {message}",
           level="INFO", encoding="utf-8")
logger.add("logs/error.log", format="{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {name}:{function}:{line} - {message}",
...
async def monitor_command(interaction: discord.Interaction, 주제: str, 기간: str = "1주일"):
    ...
    try:
        ...
        await interaction.followup.send(embed=discord.Embed(title="🔍 이슈 모니터링 시작...",
                                                            description=f"**주제**: {주제}\n**기간**: {period_description}\n\n1/3. 키워드 생성 중...",
                                                            color=0x00aaff), wait=True)

        keyword_result = await generate_keywords_for_topic(주제)

        await interaction.edit_original_response(embed=discord.Embed(title="🔍 이슈 모니터링 진행 중...",
                                                                     description=f"**주제**: {주제}\n**기간**: {period_description}\n\n2/3. 이슈 검색 및 환각 탐지 중...",
                                                                     color=0x00aaff))
...
async def thresholds_command(interaction: discord.Interaction):
    ...
    embed.add_field(name="🔍 탐지기별 최소 신뢰도",
                    value=f"• RePPL: {t.reppl_threshold:.1%}\n• 자기 일관성: {t.consistency_threshold:.1%}\n• LLM Judge: {t.llm_judge_threshold:.1%}",
                    inline=True)
    embed.add_field(name="📊 신뢰도 등급",
                    value=f"• 매우 높음: {t.very_high_boundary:.1%} 이상\n• 높음: {t.high_boundary:.1%} 이상\n• 보통: {t.moderate_boundary:.1%} 이상",
                    inline=True)
    await interaction.response.send_message(embed=embed)
